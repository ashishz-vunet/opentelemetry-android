...
/**
 * Semantic navigation direction inferred from Activity and Fragment lifecycle transitions.
 *
 * @property value Stable string written to the `navigation.transition.type` span attribute.
 */
internal enum class NavigationTransitionType(
    val value: String,
) {
    /** A new screen was shown on top of the previous one. */
    PUSH("push"),

    /** The visible screen was swapped without growing the fragment back stack (e.g. `replace`). */
    REPLACE("replace"),
